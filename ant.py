...
def get_terreno(city1, city2):
    ...
    try:
        ...
        descripcion_terreno = descripcion_tipos_de_terreno.get(terreno, 0.5)
        dificlutad_terreno = dificultad_tipos_de_terreno.get(terreno, 0.5)
...
class GeneticAlgorithm:
    def __init__(self, population_size, mutation_rate, generations, weights, pheromone_power):
    ...
    def fitness(self, weights, ants, index, distances, pheromones):
        ...
        for next_node in ants['remaining'][index]:
            ...
            inclinacion = float(get_inclination(this_node[0], next_node[0]))
            dificultad = float(get_terreno(this_node[0], next_node[0]))
    ...
    def evolve(self, ants, index, distances, weights, pheromones):
...
class AntColonySolver:
    ...
    def next_node(self, ants, index):
        ...
        ga = GeneticAlgorithm(population_size, mutation_rate, generations, weights, self.pheromone_power)

        
        # Encuentra los mejores pesos usando el algoritmo genético
        best_weights = ga.evolve(ants, index, self.distances, weights,self.pheromones)
        ...
        if(len(weights)>0):
            selcted_next_node = max(weights, key=lambda x: x[0])
            next_node=selcted_next_node[1]
        return next_node
